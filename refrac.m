...
for iElements = 1:nElements
    ...
    len = length(mfilename);
    file = [currentpath(1:end-len) 'AtomicScatteringFactor/'];
    file = [file, lower(formula.elements{iElements})];
    file = [file,'.nff'];
    try 
        fid = fopen(file);
        fgetl(fid);
...
xresult.electronDensity = 1e6*massDensity/formula.molecularWeight*AVOGADRO...
    *formula.numberOfElectrons;    % electron density, wwj, 20240105
